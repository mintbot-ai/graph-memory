...
Ladybug database and removes it afterward; the user's real graph is untouched.
"""
...
async def ensure_ladybug_indexes(driver: LadybugDriver) -> None:
    """Create the FTS indexes Graphiti's embedded search path requires.
...
async def main() -> None:
    ...
    driver = LadybugDriver(db=str(temp_dir / "graph.lbug"))
    graph = Graphiti(
        graph_driver=driver,
    ...
    try:
        await ensure_ladybug_indexes(driver)
        await graph.add_episode(
            name="database-decision",
            episode_body=(
                "Mart selected embedded LadybugDB, the maintained Kuzu continuation, "
                "for the Graph Memory project. Graph Memory is the first reference "
                "implementation of AXP."
            ),
        ...
        results = await graph.search(
            "Which maintained database does the AXP graph-memory reference implementation use?",
            group_ids=None,
            num_results=8,
        )
        ...
        for fact in facts:
            print(f"- {fact}")
        if not any("ladybug" in fact.lower() for fact in facts):
            raise AssertionError("semantic recall did not return the LadybugDB fact")
        print("LIVE_E2E_OK")
    finally:
        await graph.close()
